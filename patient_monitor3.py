...
# --- PAGE CONFIG ---
st.set_page_config(
    page_title="AI ICU Patient Monitor",
    page_icon="🏥",
    layout="wide",
...
# ============================================================
# CSS — CLEAN PROFESSIONAL MEDICAL DASHBOARD
# ============================================================
st.markdown("""
<style>
/* ===== RESET & GLOBAL ===== */
...
/* Telemetry in sidebar */
.tele-box {
    background: #f0f2f5; border: 1px solid #dde1e7; border-radius: 8px;
    padding: 8px 12px; font-size: 0.73em; margin-top: 6px;
}
...
/* ===== PATIENT BANNER ===== */
.pt-banner {
    background: #fff; border-radius: 10px; padding: 14px 20px; margin-bottom: 16px;
    box-shadow: 0 1px 6px rgba(0,0,0,0.04); border-left: 4px solid #3b82f6;
...
.pt-banner .avatar {
    ...
    font-size: 1.1em; color: #fff; font-weight: 700; flex-shrink: 0;
}
...
.vc .vr { font-size: 0.55em; color: #cbd5e1; margin-top: 4px; }
/* Normal vitals text color: make clear / dark */
.vc .vr { color: #1e293b; }

/* Vital card status colors — left border + value color */
.vc.ok   { border-left-color: #10b981; }
.vc.ok .vv { color: #059669; }
.vc.warn { border-left-color: #f59e0b; }
.vc.warn .vv { color: #d97706; }
.vc.crit { border-left-color: #ef4444; }
.vc.crit .vv { color: #dc2626; }

/* ===== CRITICAL ALERT ===== */
.crit-alert {
    background: #fef2f2; border: 1px solid #fecaca; border-left: 5px solid #ef4444;
    color: #991b1b; padding: 12px 20px; border-radius: 8px;
    font-weight: 700; font-size: 0.92em; margin-bottom: 16px;
    display: flex; align-items: center; gap: 10px;
}

/* ===== SECTION TITLE ===== */
.sec { font-size: 0.95em; font-weight: 700; color: #1e293b; margin: 20px 0 10px 0;
       padding-bottom: 6px; border-bottom: 2px solid #e2e8f0; }

/* ===== AI RESPONSE ===== */
.ai-box {
    background: #fff; border: 1px solid #e2e8f0; border-left: 4px solid #3b82f6;
    border-radius: 8px; padding: 16px 20px; margin: 12px 0;
    font-size: 0.88em; line-height: 1.75; color: #334155;
...
.stButton > button {
    ...
    font-size: 0.82em !important; letter-spacing: 0.2px;
    transition: all 0.2s;
}
...
def run_agent(fp, query):
    ...
    try:
        llm = None
        # Try multiple import paths to handle proxies bug
        ...
        agent = create_csv_agent(llm, fp, verbose=False, agent_type="openai-functions")
        t0 = time.time()
        resp = agent.invoke(query)
        if isinstance(resp, dict): resp = resp.get("output", str(resp))
        lat = round(time.time()-t0, 2)
        toks = len(str(query)+str(resp))//4
        st.session_state.telemetry = {"model":"GPT-4o","latency":f"{lat}s","tokens":f"~{toks}","status":"Active","ts":datetime.now().strftime('%H:%M:%S')}
        return resp
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<p style='text-align:center;font-size:0.72em;color:#94a3b8;'>ICU Monitor v2.0 · GPT-4o</p>", unsafe_allow_html=True)
...
# — Vital Cards (pure HTML grid — no Streamlit columns) —
st.markdown(f'<div class="sec">🫀 Current Vital Signs</div>', unsafe_allow_html=True)
st.markdown(f"""
<div class="vc-row">
...
# — Trend Charts — (separate charts for HR, SpO₂, Temp)
...
font_small = dict(family='Arial', size=11, color='#1e293b')

tc1, tc2, tc3 = st.columns(3)
with tc1:
    fhr = go.Figure()
    fhr.add_trace(go.Scatter(
        x=df['timestamp'], y=df['heart_rate_bpm'], name='Heart Rate',
        line=dict(color='#ef4444', width=3), line_shape='spline', marker=dict(size=3)
    ))
    # normal band 60-100
    fhr.add_hrect(y0=60, y1=100, fillcolor="#10b981", opacity=0.06, line_width=0)
    fhr.update_layout(title="Heart Rate", template='plotly_white', height=320, margin=chart_margin,
                      yaxis=dict(title="BPM"), hovermode='x unified', font=font_small)
    st.plotly_chart(fhr, width='stretch')
with tc2:
    fsp = go.Figure()
    fsp.add_trace(go.Scatter(
        x=df['timestamp'], y=df['spo2_percent'], name='SpO₂',
        line=dict(color='#2563eb', width=3), line_shape='spline', marker=dict(size=3), fill='tozeroy', fillcolor='rgba(59,130,246,0.06)'
    ))
    # normal band >=95
    fsp.add_hrect(y0=95, y1=100, fillcolor="#10b981", opacity=0.06, line_width=0)
    fsp.update_layout(title="SpO₂", template='plotly_white', height=320, margin=chart_margin,
                      yaxis=dict(title="%", range=[max(80, df['spo2_percent'].min()-2), 100]), hovermode='x unified', font=font_small)
    st.plotly_chart(fsp, width='stretch')
with tc3:
    ftemp = go.Figure()
    ftemp.add_trace(go.Scatter(
        x=df['timestamp'], y=df['temperature_c'], name='Temperature',
        line=dict(color='#f59e0b', width=3), line_shape='spline', marker=dict(size=3)
    ))
    # normal band 36.1 - 37.9
    ftemp.add_hrect(y0=36.1, y1=37.9, fillcolor="#10b981", opacity=0.06, line_width=0)
    ftemp.update_layout(title="Temperature", template='plotly_white', height=320, margin=chart_margin,
                        yaxis=dict(title="°C"), hovermode='x unified', font=font_small)
    st.plotly_chart(ftemp, width='stretch')
...
with c2:
    submit_query = st.button("🔍 Ask AI", width='stretch')
if submit_query and q.strip():
    with st.spinner("Analyzing…"):
        r = run_agent(fp, f"Patient: {pt['name']}, {pt['age']}y, Allergies: {pt['allergy']}, Dx: {pt['dx']}. {q}. Keep response concise and under 200 words.")
    st.markdown(f'<div class="ai-box">{r}</div>', unsafe_allow_html=True)

cc1, cc2 = st.columns(2)
with cc1:
    fbp = go.Figure()
    fbp.add_trace(go.Scatter(
        x=df['timestamp'], y=df['bp_systolic_mmHg'], name='Systolic',
        line=dict(color='#ef4444', width=3), line_shape='spline', marker=dict(size=3)
    ))
    fbp.add_trace(go.Scatter(
        x=df['timestamp'], y=df['bp_diastolic_mmHg'], name='Diastolic',
        line=dict(color='#8b5cf6', width=3), line_shape='spline', marker=dict(size=3)
    ))
    fbp.add_hrect(y0=90, y1=140, fillcolor="#10b981", opacity=0.04, line_width=0)
    fbp.update_layout(title="Blood Pressure", template='plotly_white', height=320, margin=chart_margin,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), yaxis=dict(title="mmHg"), font=font_small)
    st.plotly_chart(fbp, width='stretch')
with cc2:
    ec = df['ECG'].value_counts()
    fec = go.Figure(data=[go.Pie(
        labels=ec.index, values=ec.values, hole=0.45,
        marker=dict(colors=['#10b981','#f59e0b','#ef4444','#8b5cf6','#f97316'][:len(ec)], line=dict(color='#ffffff', width=1)),
        textinfo='percent+label'
    )])
    fec.update_layout(title="ECG Distribution", template='plotly_white', height=320, margin=chart_margin, font=font_small)
    st.plotly_chart(fec, width='stretch')
...
with b1:
    if st.button("📊 Trend Analysis", width='stretch'):
        with st.spinner("…"):
            r = run_agent(fp, "60-minute vital sign trend analysis. Be concise, under 150 words. Identify deterioration patterns.")
        st.markdown(f'<div class="ai-box">{r}</div>', unsafe_allow_html=True)
with b2:
    if st.button("⚠️ Risk Score", width='stretch'):
        with st.spinner("…"):
            r = run_agent(fp, "Risk stratification: rate LOW/MODERATE/HIGH/CRITICAL. Give 3 reasons. Under 100 words.")
        st.markdown(f'<div class="ai-box">{r}</div>', unsafe_allow_html=True)
with b3:
    if st.button("💊 Medications", width='stretch'):
        with st.spinner("…"):
            r = run_agent(fp, f"Suggest medications. Allergies: {pt['allergy']}. Under 120 words. Be specific with dosages.")
        st.markdown(f'<div class="ai-box">{r}</div>', unsafe_allow_html=True)
with b4:
    if st.button("📋 SBAR Handoff", width='stretch'):
        with st.spinner("…"):
